...
public class OperaErrorMonitor {
	...
	OperaErrorMonitor() throws Exception {
		...
		path=(path==null||path.isEmpty() || !(new File(path)).exists())?"./" : path;
		...
		if (MONITOR_LOG_FILE.length() / 1024 / 1024 > 5) {
			MONITOR_LOG_FILE.renameTo(new File(MONITOR_LOG_FILE.getPath().replace(
			      ".log", ".bak")));
		...
		try {
			...
			if (errorNotificationDelay == null)
				throw new Exception("The ini file does not contain"
				      + MONITOR_PARAMETER_NOTIFICATION_DELAY);
			else window.setErrorNotificationDelay(Double
			...
			else window.setDelay(Long.valueOf(delay));
			...
			if (resendDelay == null)
				throw new Exception("The ini file does not contain"
				      + MONITOR_PARAMETER_RESEND_DELAY);
			else window.setResendDelay(Long.valueOf(resendDelay) * 60);
			...
			if (maxNumNotif == null)
				throw new Exception("The ini file does not contain"
				      + MONITOR_PARAMETER_MAX_NUM_NOTIFCATION);
			else window.setMaxNumNotifications(Integer.valueOf(maxNumNotif));
	...
	static void error(String error, JFrame jFrame) {
		...
		writeLog(DateUtils.now(OperaLogsParsers.timeFormat)+"- MONITORING TOOL ERROR - Message= "+error.replaceAll("\r\n", " "), null);
		if (jFrame == null) {
			try {
