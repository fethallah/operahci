...
If ($CmdLine[0] = 3 And $UserID[0] = 2) Then
	$runErrors = RunAs($UserID[2], $UserID[1], $CmdLine[3], 2, 'Shutdown /r /f /t 0 /d u:4:5 /m ' & $CmdLine[1], @SystemDir, @SW_HIDE, $STDERR_CHILD + $STDOUT_CHILD)
	ConsoleWrite('Shutdown /r /f /t 0 /d u:4:5 /m ' & $CmdLine[1] & ' User:' & $UserID[2] & ' Domain: ' & $UserID[1])
Else
	$runErrors = Run('Shutdown /r /f /t 0 /d u:4:5 /m ' & $CmdLine[1], @SystemDir, @SW_HIDE, $STDERR_CHILD + $STDOUT_CHILD)
