...
namespace NIMacro {
	...
	*/
	...
	public: // static interface
		...
		// Check if a memblock class has been registered or was present in the guide file
		static bool IsClassKnown(const safestring& classname);
	...
	private: // friend interface
		...
		* @param mx_level Used in Debug builds for mutex lock order consistency checks.
		*/
		Package(const safestring& libdir, int mx_level);
	...
	private: // static data
		static boost_mutex g_classes_mx_;
		static std::set<safestring> g_classes_;
	...
	*/
	...
	public:
		...
		/**
		* Return the package corresponding to the specified StdLib or UserLib directory.
		* The Package object is created if not yet existing.
		* @param libdir Absolute directory name (local or UNC). Can contain backslashes and can end by a slash.
		* @param mx_level Used in Debug builds for mutex lock order consistency checks.
		*/
		PPackage GetPackage(const safestring& libdir, int mx_level);
