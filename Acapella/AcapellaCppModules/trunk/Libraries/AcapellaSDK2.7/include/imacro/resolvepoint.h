...
//		static void FlushAllParsedSubscripts(std::set<ResolvePoint*>& processed);
	...
	public: // interface
		...
		*/
		...
		/// For debug-build mutex lock consistency checks
		bool IsLockedByThisThread() const {return mx_.IsLockedByThisThread();}
#endif
	...
	private: // data protected by mx_
		...
		/// A list of modules whose search is known to fail in this resolvepoint (fb #33979). The module names are encoded here as "PROPERNAME/PACKAGE".
		std::set<safestring> known_missing_modules_;
