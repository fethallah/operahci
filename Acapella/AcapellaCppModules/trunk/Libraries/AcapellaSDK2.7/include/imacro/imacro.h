...
* @param versioned Specifies whether to use version-specific Acapella registry branch or not.
* @return The value found in registry, or an empty string if not found.
*/
DI_IMacro safestring GetAcapellaRegKey(const safestring& subkey_name, const safestring& value_name, bool versioned=true);
...
/**
* Returns a pointer to the entry k in the current IMacro list of UserLib abd StdLib directories.
...
* This forwards to MemBlock::Factory().GetInstance(). If this fails and try_to_load=true is passed,
* the corresponding module library is attempted to load with the help of the ResolvePoint/Package class mechanisms.
* One can specify try_to_load=true only if no ResolvePoint classes are currently locked, in order to avoid
* inconsistent mutex lock order.
*/
DI_IMacro PMemBlock GetMemBlockClassInstance(const safestring& classname, PResolvePoint resolvepoint=NULL, bool try_to_load=true);
...
/**
* Returns Acapella __bindir__ constant (the directory of IMacro DLL).
*/
DI_IMacro Nbaseutil::safestring GetBinDir();
...
* @param version Pass a general product version, two or three parts, e.g. "2.2", or "2.3.0"
*/
...
/**
* Returns the product version as set by SetProductVersion() or guessed, plus optionally two last parts of Acapella version. 
* If not set, tries to guess the product version by the current product name and current Acapella version. 
* If guessing fails, an empty string is returned.
* @param add_acapella_version_tail Add parts of the current Acapella version to the product version.
*              This makes it possible to recreate the full Acapella version as the first two Acapella version parts are
*              in 1:1 correspondence with the product version (see http://wiki/Acapella/AcapellaVersions).
*			   The Acapella version parts are added only if they are not present in the product version as set by SetProductVersion().
* @return A product version like "2.3" or a combination of product and Acapella versions, e.g. "2.2.6.65783".
*/
DI_IMacro Nbaseutil::safestring GetProductVersion(bool add_acapella_version_tail=true);
...
public: // virtual interface
	...
	*/
	...
	/// Return an informal name for the cacher object.
	virtual safestring GetName() const=0;
...
/// Encrypt a short string by Acapella public RSA key.
DI_IMacro safestring EncryptWordForAcapella(const safestring& plaintext, const safestring& challenge);

/// Encrypt a large string by Acapella public RSA key.
DI_IMacro void EncryptFileForAcapella(const safestring& plainText, safestring& cipherText_out);
