...
namespace NIMacro {
	...
	*/
	...
	public:
		/// The timer facility will call the callback roughly after each sleep seconds has been passed. Pass 0 to wait indefinitely.
		...
		*/
		void Start(bool trigger_first_call_immediately=false);
		/// Stop() must be called before destructing the most derived object, or from the destructor body of the most derived object.
		void Stop();
		/// Change the interval to wait between timer callback calls. Pass 0 to wait indefinitely.
		void ChangeInterval(int sleep_seconds);
