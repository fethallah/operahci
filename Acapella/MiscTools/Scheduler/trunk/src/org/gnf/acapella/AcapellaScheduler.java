...
public class AcapellaScheduler {
	...
	private static String instrument = "opera";
	...
	public static void main(String[] args) throws Exception {
		...
		CmdLineParser.Option errorFileName = parser.addStringOption('e',
				"errorFile");
		CmdLineParser.Option instrument = parser.addStringOption('i',
				"instrument");
		...
		value = (String) parser.getOptionValue(instrument,
				AcapellaScheduler.instrument);
		setInstrument(value);
	...
	AcapellaScheduler() throws Exception {
		...
		ErrorExporter errrorExporter = new ErrorExporter(getErrors(),
				getErrorFile());
		System.out.printf("Errors logged in:\t%1$s%n%n", getErrorFile()
				.getPath());
		...
		if (fileSet == null || fileSet.isEmpty())
			error(new Exception(String.format(
							"Nothing to do. You may not have selected a valid instrument for these images, or the image path: %s is empty.",
					getImagesPath())), false);


		System.out
				.println("Num/Tot\tPath\tThread\tWellProcessed\tNumProc\tAvgTimePerWell\tTotalWell");
		int fileIndex = 0;
		int totalWellsRead = 0;
		for (File path : fileSet.keySet()) {
			...
			int batchSize = Math.min(
					...
					(int) Math.ceil((double) numberOfWells
							/ (double) numProcessor));
			System.out.printf("%1$04d/%2$04d\t%3$s \t", fileIndex,
					fileSet.size(), path);
			...
			totalWellsRead += numberOfWells;
			System.out.printf("\t%1$04d\t%2$03d\t%3$8.2f\t%4$08d%n",
					numberOfWells, numProcessor, time, totalWellsRead);
		}
	...
	private static void printUsage() {
		...
		System.err
				.println("Usage: AcapellaScheduler [-s,--script] [{-p,--param}] [{-r,--resultPath}]\r\n"
						+ "\t\t[{-d,--dataFile}] [{-e,--errorFile}] [{-i,--instrument}] [{-O,--overWrite}]\r\n"
						+ "\t\t[{-m,--wellMask} a String] [{-b,--batchSize} an Integer] [{-t, --threads} an Integer]\r\n"
						+ "\t\t[Path to Your images]\r\n"

						+ "\r\n\t[-s,--script]: Path to your script."
						+ "\r\n\t[-p,--param]: Path to your script parameters."
						...
						+ "\r\n\t[{-i,--instrument}]: Name of the instrument used to read the images (Optional, default= 'opera', must be one of: 'opera', 'incell',)."
	...
	private static void setInstrument(String instrument) {
		AcapellaScheduler.instrument = instrument.toLowerCase();
